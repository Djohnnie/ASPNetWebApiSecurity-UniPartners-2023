...
builder.Services.ConfigureEncryptedIdentity(builder.Configuration);
...
{
    return helper.Post(x => x.LoginWithRefresh(request));
}).WithName("Login").WithOpenApi().AllowAnonymous();


app.MapPost("/refresh", (RefreshRequest request, IApiHelper<IdentityManager> helper) =>
{
    return helper.Post(x => x.RefreshToken(request));
}).WithName("Refresh").WithOpenApi().AllowAnonymous();


app.MapPost("/revoke", (IApiHelper<IdentityManager> helper) =>
{
    return helper.Execute(x => x.RevokeRefreshToken());
}).WithName("Revoke").WithOpenApi().RequireAuthorization();
